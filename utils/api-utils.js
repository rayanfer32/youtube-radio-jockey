...
const APIUtils = {
  ...
  generateRJPrompt(currentSong, nextSong, style = 'energetic', length = 'medium') {
    const stylePrompts = {
...
Create engaging commentary that connects with listeners. Be natural, enthusiastic, and add personality. Don't just read the song titles - make it conversational and fun!`;
  ...
  // Rate limiting check
  async checkRateLimit(service, limit = 60) {
    return new Promise((resolve) => {
      chrome.runtime.sendMessage(
        { action: 'checkAPILimit', service, limit },
        (response) => resolve(response?.canMakeRequest || false)
      );
    });
  }
};
